...
def main():
    ...
    dataset = load_dataset("tokenized_ds")

    def add_token_length(batch):
        # batch["attention_mask"] 는 각 예시별 0/1 리스트
    ...
    filtered_ds = dataset.filter(filter_not_truncated)
    ...
    args = TrainingArguments(
        output_dir="koelectra-detector_7",
        per_device_train_batch_size=78//2,
        per_device_eval_batch_size=64,
        gradient_accumulation_steps=8, # koelectra_9000: 4, koelectra42_4000: 8
        eval_strategy="steps",
        eval_steps=500,
        save_steps=1000,
        logging_steps=200,
        ...
        bf16=True,
        seed=SEED, # koelectra_9000: 시드 고정 X
    )
